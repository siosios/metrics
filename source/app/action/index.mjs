...
;(async function() {
  try {
    ...
    if (!token)
      throw new Error("You must provide a valid GitHub personal token to gather your metrics (see https://github.com/siosios/metrics/blob/master/.github/readme/partials/documentation/setup/action.md for more informations)")
    ...
    //Test token validity and requests count
      ...
      try {
        ...
        if (!("x-oauth-scopes" in headers)) {
          throw new Error(
            'GitHub API did not send any "x-oauth-scopes" header back from provided "token". It means that your token may not be valid or you\'re using GITHUB_TOKEN which cannot be used since metrics will fetch data outside of this repository scope. Use a personal access token instead (see https://github.com/siosios/metrics/blob/master/.github/readme/partials/documentation/setup/action.md for more informations).',
          )
        }
    ...
    if (_notice_releases) {
      const {data: [{tag_name: tag}]} = await rest.repos.listReleases({owner: "siosios", repo: "metrics"})
